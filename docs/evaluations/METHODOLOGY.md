...
실제 Trial은 결과가 나오는 것뿐 아니라 반복 가능한 개발 속도도 중요하다. 운영 목표는
**제출 시작부터 terminal 상태까지 20분 이내**, 절대 상한은 **30분**으로 둔다.

- 모델 처리 구간은 플랫폼 준비·전송·채점 여유를 남기기 위해 약 15분을 목표로 한다.
- 20분을 넘기면 latency 회귀로 경고하고, 30분을 넘기거나 timeout이면 승격하지 않는다.
- 로컬 `coverage-v2`의 420초 제한은 실제 Trial 시간의 예측식이 아니라 더 빠르고 엄격한
  사전 선별 gate다.
- Trial 기록에는 제출 시각, 평가 시작 시각을 알 수 있으면 시작 시각, terminal 시각과
  최종 상태를 함께 남겨 모델 시간과 플랫폼 대기를 구분한다.
- 개별 요청 제한을 늘려 완주를 시도하지 않는다. D5의 실제 성공 기준인 요청 90초,
  upstream 30초와 충분한 동시성을 우선 기준으로 삼는다.
