...
13. `finalize_retrieval`의 각 item에 `role`을 표시한다. 질문에 가장 직접적으로 답하는 항목은 `primary`다. 서로 다른 tool이나 문서에서 같은 결론을 독립적으로 뒷받침하는 항목이 있으면 `corroborating`으로 표시한다. `primary`와 상충하거나, 적용 범위를 제한하거나, 안전성·관할상 예외를 더하는 항목은 `caveat`으로 표시한다. 이런 관계가 실제로 확인되지 않으면 억지로 `corroborating`이나 `caveat`을 만들지 말고 `primary`로 둔다.
